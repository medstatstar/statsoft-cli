# Statistica 检测与手动指引脚本 / Statistica detection & manual-guidance script
# 支持平台: Windows-only
#
...
function Write-Info {
    ...
    Write-Lang "[INFO] $Message" "[INFO] $Message" -ForegroundColor Green
}
function Write-Warn {
    param([string]$Message)
    Write-Lang "[WARN] $Message" "[WARN] $Message" -ForegroundColor Yellow
}
function Write-Error {
    param([string]$Message)
    Write-Lang "[ERROR] $Message" "[ERROR] $Message" -ForegroundColor Red
}

# 检测 Statistica / Detect Statistica
...
# 验证 Statistica / Verify Statistica (read-only)
...
# 打印手动指引 / Print manual guidance (no config write)
function Print-ManualGuidance {
    ...
    Write-Info "  • GUI: 直接打开 Statistica（图形界面）/ Open Statistica GUI directly"
...
function Main {
    ...
    if (-not $statisticaPath) {
        ...
        # 非交互回退 — Read-Host 带超时 / Non-interactive fallback
        $userPath = $null
        try {
            $userPath = Read-Host "[CN] 请输入 Statistica 安装路径（按 Enter 跳过）/ [EN] Enter Statistica installation path (press Enter to skip)"
        }
        catch {
            Write-Lang "[!] 非交互模式，跳过手动输入 / Non-interactive mode, skipping manual input" "[!] Non-interactive mode, skipping manual input" -Color Yellow
        }
        ...
        if ($userPath) {
            ...
            Write-Info "提示: 本脚本仅检测，不写入配置。如需执行请手动启动 Statistica 或使用受控运行器。"
            Write-Info "Hint: this script only detects; it writes no config. To execute, launch Statistica manually or use the guarded runner."
            exit 1
        }
    }
